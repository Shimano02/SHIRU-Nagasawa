...
async function processInput(inputText, audioFile, uploadedFileId = null) {
  try {
    ...
    if (botReply) playAvatarWithResponse(botReply)
...
async function sendMessage(userInput, files = []) {
  try {
    ...
    guessMediaCitations(answerBuf);
    /* モーダルが表示中なら履歴をリロード */
    if (historyModalEl?.style.display === "flex" && conversationId) {
      populateHistoryModal(conversationId).catch(() => {});
    }
    return lastBotResponse;
...
function attachTTSButton(botDiv, markdownText){
  ...
  btn.addEventListener('click', async e=>{
    ...
    try{
      await playAvatarWithResponse(markdownText);    // ← 動画＋音声
...
try {
  ...
  if (historyBtn && historyModal) {
    historyBtn.addEventListener("click", async () => {
      ...
      if (conversationId) await fetchConversationHistory(conversationId);
      else if (historyListEl) historyListEl.innerHTML = "<li>まだ会話がありません</li>";
    });
  }
  ...
  if (historyLink && historyModal) {
    historyLink.addEventListener("click", async (e) => {
      e.preventDefault();
      historyModal.style.display = "flex";
      await fetchConversationHistory();              // convId 未指定
    });
  }
  ...
  async function displayFileList(difyData) {
    /* ---- Dify ＋ メディアを結合 ---- */
    const mediaData = await fetchMediaList();          // [{id,title,attached_file}, …]
    const joined = [
      ...(difyData.data || []).map(d => ({ ...d, _src: "dify" })),
      ...(mediaData || []).map(m => ({ ...m,  _src: "media" }))
    ];
...
async function fetchSuggestedQuestions(messageId, maxRetry = 5) {
  ...
  try {
    ...
    const userEmail = localStorage.getItem("userEmail") || "anonymous";
    const resp = await apiFetch(
      `/messages/${messageId}/suggested?user=${encodeURIComponent(userEmail)}`
    );
...
function loginSuccess(data) {
  ...
  localStorage.setItem("userTokenBalance",
    (data.user.products ? data.user.total : data.user.token_balance) ?? 0);
...
async function fetchRemainingTokens() {
  try {
    ...
    if (resp.status === 401) {
      /* balance エンドポイントは一瞬 401 を返すことがあるので
         まずリフレッシュだけ試み、画面はそのまま */
      const fails = (+localStorage.getItem(failKey) || 0) + 1;   // ← 加算演算子を補完
      localStorage.setItem(failKey, fails);
      if (fails > 3) {
        // 30 分キャッシュして再度 401 スパムを防止
        apiCache.set(cacheKey, null, 30*60*1000);
      } else {
        await tryRefresh();
      }
      return null;
    ...
    apiCache.set(cacheKey, balObj, 5 * 60 * 1000);   // 5 分キャッシュ
    return balObj;
  } catch (error) {
...
async function consumeTokens(amount) {
  ...
  try {
    const resp = await apiFetch(`${API_BASE}/app/api/tokens/consume`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
    ...
    if (resp.status === 401) {                   // 失効 or race
      const refreshed = await tryRefresh();
      if (refreshed) {
        /* 1 回だけリトライ → 成功すれば以後 OK */
        return await consumeTokens(amount);
      }
      console.warn("consumeTokens: auth failed ? 残高は未更新");
      return;
    }
    if (!resp.ok) {
...
async function checkSubscriptionStatus() {
  ...
  try {
    const resp = await apiFetch(`${API_BASE}/app/api/subscription/status`, {
      method: "GET",
      headers: {
        "Authorization": `Bearer ${token}`
      }
...
/* ====== トークン残高バッジ (#token-balance) 操作 ====== */
function setGlobalTokenBadge(balance){
  const badge = document.getElementById("token-balance");
  if (!badge) return;                          // HTML が無ければ終了

  /* balance がオブジェクト { total, … } なら total を使用 */
  const val = (balance && typeof balance === "object")
              ? balance.total
              : balance;
...
function updateBalanceDisplay(raw) {
  ...
  const tot = document.getElementById("user-token-balance");
  if (tot)   tot.textContent = bal.total;

    /* ★ ヘッダー右上バッジも更新 */
    setGlobalTokenBadge(bal.total);
    
    // キャッシュを更新
    apiCache.set("token-balance", bal, 5 * 60 * 1000);
    if (bal.total === 1 && !sessionStorage.getItem("lowTokenAlertShown")) {
      alert("残り使用回数が1になりました。0になると自動チャージされます。");
      sessionStorage.setItem("lowTokenAlertShown", "1");
    }
}
...
async function apiFetch(url, options = {}) {
  ...
  /*** 認証ヘッダ（明示的に auth:false を渡した場合は付けない） ***/
  if (opt.auth !== false) {
    const token = localStorage.getItem(TOKEN_KEY);
...
function setupUnauthorizedInterceptors() {
  ...
  if (userInput) {
    ...
    userInput.addEventListener("keydown",
       unauthorizedKeydownHandler, true);
  }
}
...
function enableUserInteractions() {
  ...
  if (inputField) {
    ...
    if (unauthorizedKeydownHandler) {                   // ② 解除
      inputField.removeEventListener(
          "keydown", unauthorizedKeydownHandler, true);
...
function stopAvatarPlayback(){
  if(avatarVideo){
    ...
    avatarVideo.load();
    avatarVideo.setAttribute("poster","avatar.jpg");
...
/* ---------------------------------------------------------
   メディアナレッジ（画像/動画/PDF）一覧を取得
   ---------------------------------------------------------*/
async function fetchMediaList() {
  const cacheKey = 'media-file-list';
  ...
  const resp = await apiFetch(MEDIA_API_BASE, { method:"GET" });
  if (!resp.ok) {
    console.warn("Media list error:", await resp.text());
    return [];
  }
  /* 常に配列を返すよう整形 */
  ...
  apiCache.set(cacheKey, list, 5*60*1000);    // 5分キャッシュ
  return list;
}
...
document.getElementById("open-knowledge-btn").addEventListener("click",openKnowledgeModal);

document.getElementById("kn-submit-btn").addEventListener("click", async ()=>{
  const title   = document.getElementById("kn-title").value.trim();
  if(!title){alert("タイトルは必須です");return;}

  const content = document.getElementById("kn-content").value;
  const fileEl  = document.getElementById("kn-file");
  ...
  const tenantId = parseInt(localStorage.getItem("userTenant"), 10) || 1;
  fd.append("tenant", tenantId);
  if(file) fd.append("attached_file", file);

  try{
     const res = await apiFetch(MEDIA_API_BASE,{method:"POST",body:fd});
     if(!res.ok) throw new Error(await res.text());
     alert("ナレッジを登録しました");
     apiCache.clear("media-file-list");           // 一覧リロード用
     closeKnowledgeModal();
  }catch(e){
...
async function populateHistoryModal(convId){
  ...
  if(Array.isArray(data.data) && data.data.length){
    data.data.forEach(msg=>{
      const li = document.createElement("li");
      li.innerHTML =
        `<strong>あなた：</strong> ${DOMPurify.sanitize(msg.query||"")}<br>`+
        `<strong>ADO：</strong> ${DOMPurify.sanitize(msg.answer||"")}`;
